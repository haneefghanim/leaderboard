...
Leaderboard.prototype.create = function(board, user, callback) {
	...
	this.client.exists(board, function (err, value) {
	    ...
	    if (value == true) {
	    	...
	    	that.client.zadd(board, 1, user); // add user to new board
	    	that.client.sadd(that.leaderKey, board); // add board to list of boards
...
Leaderboard.prototype.delete = function(board, callback) {
	...
	this.client.exists(board, function (err, value) {
		if (err) throw(err)
		if (value == true) {
			that.client.del(board); // delete board
			that.client.srem(that.leaderKey, board); // remove board from list of boards
...
Leaderboard.prototype.addUserToBoard = function(board, user, callback) {
	...
	this.client.zrevrangebyscore([board, "+inf", "-inf", 'WITHSCORES'], function (err, list) {
		...
		if (list.length > 1) {
			...
			that.client.zscore(board, user, function (err, score) {
				if (err) throw(err)
				if (score > 0) {
					callback(false, "Whoops, *" + user + "* is already in leaderboard *" + board +"*.");
				} else {
			    	redisClient.zadd(board, maxScore+1, user); // add user to board
	...
	this.client.zcard(board, function (err, length) {
		...
		if (length > 0) {
			...
			that.client.zscore(board, user, function (err, score) {
				if (err) throw(err)
				if (score > 0) {
					that.client.zrem(board, length+1, user); // remove user from board
...
Leaderboard.prototype.display = function (board, callback) {
	...
	this.client.exists(board, function (err, value) {
	    if (err) throw(err)
	    if (value == true) {
	    	that.client.zrangebyscore([board, "-inf", "+inf"], function (err, list) {
		...
		if (list.length > 1) {
			...
			if (winnerIndex >= 0 && loserIndex >= 0) {
				...
				if (winnerIndex > loserIndex) {
					...
					for (i = 0; i < list.length; i++) {
						that.client.zadd(board, i+1, list[i]);
					}

					callback(true, "*" + winner + "* took position #"+(loserIndex+1)+" from *" + loser + "* in *"+ board + "*!");
				} else {
					// Don't rearrange
					callback(true, "*" + winner + "* defended position #"+(winnerIndex+1)+" against *" + loser + "* in *"+ board + "*!");
				}
			} else {
